...
public class Code extends JFrame implements GLEventListener, MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    // Objects and Models
    private ImportedModel Fish = new ImportedModel("fish.obj");
    ...
    // Code
    public Code() {
        this.setTitle("Daniel Bui - CSC 155! - A4");
        setSize(800, 800);
        ...
        FPSAnimator animator = new FPSAnimator(myCanvas, 120);
        animator.start();
    ...
    public void firstPass(GLAutoDrawable d) {
        ...
        */
        ...
        gl.glDrawArrays(GL_TRIANGLES, 0, Fish.getNumVertices());
    }
    ...
    public void init(GLAutoDrawable d) {
        ...
        brickTexture = tr.loadTexture(d, "src/textures/brick.png");
    ...
    private void setupVertices(GL4 gl) {
        int numberFishIndices;
        int[] FishIndices = Fish.getIndices(), torusIndices = torus.getIndices(), sphereIndices = sphere.getIndices();
        Vertex3D[] FishVertices = Fish.getVertices(), torusVertices = torus.getVertices(), sphereVertices = sphere.getVertices();

        numberFishIndices = Fish.getNumIndices();

        // The Fish
        float[] pp = new float[numberFishIndices * 3];
        float[] pt = new float[numberFishIndices * 2];
        float[] pn = new float[numberFishIndices * 3];
        float[] pc = new float[numberFishIndices * 3];

        for (int i = 0; i < numberFishIndices; i++) {
            pp[i * 3] = (float) (FishVertices[FishIndices[i]]).getX();
            pp[i * 3 + 1] = (float) (FishVertices[FishIndices[i]]).getY();
            pp[i * 3 + 2] = (float) (FishVertices[FishIndices[i]]).getZ();

            pt[i * 2] = (float) (FishVertices[FishIndices[i]]).getS();
            pt[i * 2 + 1] = (float) (FishVertices[FishIndices[i]]).getT();

            pn[i * 3] = (float) (FishVertices[FishIndices[i]]).getNormalX();
            pn[i * 3 + 1] = (float) (FishVertices[FishIndices[i]]).getNormalY();
            pn[i * 3 + 2] = (float) (FishVertices[FishIndices[i]]).getNormalZ();
        }

        for (int i = numberFishIndices / 2; i < numberFishIndices; i++) {
            pc[i * 3] = 0.0f;
            pc[i * 3 + 1] = 1.0f;
            pc[i * 3 + 2] = 0.0f;
        }
